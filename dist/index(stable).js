...
import { ActionRowBuilder, ButtonBuilder, ButtonStyle, Client, StringSelectMenuBuilder, StringSelectMenuOptionBuilder, EmbedBuilder, GatewayIntentBits, ModalBuilder, REST, Routes, SlashCommandBuilder, TextInputBuilder, TextInputStyle } from 'discord.js';
...
const availabilityDayQueue = new Map();
const availabilityCurrentDay = new Map();
...
function isSlotExpiredToday(slot, now = new Date()) {
...
async function pruneExpiredSlots() {
    try {
...
function buildCoachEmbed(coach) {
    const activeSlots = getActiveSlots(coach);
    const embed = new EmbedBuilder()
        .setTitle(coach.username)
        .setDescription(coach.bio || 'No bio provided yet.')
        .addFields({ name: 'Specialties', value: coach.specialties.join(', ') || 'Not listed', inline: false }, { name: 'Open Slots', value: activeSlots.join('\n') || 'No slots available', inline: false }, { name: 'Visibility', value: coach.visibility === 'all' ? 'All servers' : 'This server only', inline: false })
        .setColor(0x5865f2);
    if (coach.avatarUrl) {
...
function buildSlotSelectMenu(coach) {
    const activeSlots = getActiveSlots(coach);
...
function buildDaySelectionMenu() {
    const row = new ActionRowBuilder();
    const menu = new StringSelectMenuBuilder()
        ...
        .addOptions(...availabilityDays.map((day) => new StringSelectMenuOptionBuilder()
        .setLabel(day)
        .setValue(day)));
    row.addComponents(menu);
    return [row];
}
function buildTimeSelectionMenu(day) {
    const row = new ActionRowBuilder();
    const menu = new StringSelectMenuBuilder()
...
function buildAvailabilityCompleteButtons() {
    const row = new ActionRowBuilder();
...
async function replyWithCoachCard(interaction, coach) {
    const embed = buildCoachEmbed(coach);
    const components = buildSlotSelectMenu(coach);
    await interaction.reply({ embeds: [embed], components, ephemeral: true });
}
async function replyWithCoachList(interaction, coaches) {
    if (coaches.length === 0) {
...
function isCoachVisible(coach, guildId) {
    return coach.visibility === 'all' || (Boolean(guildId) && coach.guildId === guildId);
}
function buildRequestResponseButtons(requestId) {
    const row = new ActionRowBuilder();
    row.addComponents(new ButtonBuilder()
...
function buildRestoreAvailabilityButtons() {
    const row = new ActionRowBuilder();
    row.addComponents(new ButtonBuilder()
...
async function sendDmToUser(userId, guildId, payload) {
    const user = await client.users.fetch(userId);
    const dm = await user.createDM();
    await dm.send(payload);
}
async function notifyCoachOfRequest(coach, requesterUsername, slot, requestId, fallbackChannelId, guildId) {
...
async function notifyRequesterOfDecision(requesterId, coachUsername, slot, accepted, reason, requestInteraction, guildId) {
...
client.on('interactionCreate', async (interaction) => {
    ...
    if (interaction.isButton()) {
        ...
        if (type === 'availability-finish') {
            ...
            const normalizedSlots = Object.entries(dayTimes).flatMap(([day, times]) => times.map((time) => `${day} ${time}`));
            const data = await readData();
