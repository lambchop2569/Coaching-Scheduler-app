...
export interface CoachProfile {
    ...
    timezone?: string;
    /** Local calendar date (YYYY-MM-DD) on which saved slots should be restored. */
    scheduledRestoreDate?: string;
...
export interface PendingAvailability {
    ...
    timezone?: string;
    guildId?: string;
    /** Set when changing slots for an existing profile. */
    coachId?: string;
}

export interface PendingScheduleSelection {
    coachId: string;
    day: string;
    slot: string;
    coachUsername: string;
...
export const pendingAvailability = new Map<string, PendingAvailability>();
export const pendingScheduleSelections = new Map<string, PendingScheduleSelection>();
export const availabilityByDay = new Map<string, Record<string, string[]>>();
export const availabilityDayQueue = new Map<string, string[]>();
export const availabilityCurrentDay = new Map<string, string>();
export const coachListMessages = new Map<string, {
    messageIds: string[];
...
export function buildDatedSlot(template: string, now = new Date()) {
    const [day, ...timeParts] = template.split(' ');
    const time = timeParts.join(' ');
    const weekday = WEEKDAY_NAMES.indexOf(day);

    if (weekday === -1 || !parseSlotTime(time)) return template;
...
export function isSlotExpiredToday(slot: string, now = new Date()) {
    ...
    if (datedMatch) {
        ...
        if (!parsed || month === -1) return false;

        const slotDate = new Date(
            Number(yearText), month, Number(dayText), parsed.hour, parsed.minute
        );
...
function timestampForSlot(slot: string, timezone: string) {
...
function formatSlotsForViewer(slots: string[], timezone: string) {
...
function formatSlotForPlayer(slot: string, coachTimezone: string, playerTimezone: string) {
    const timestamp = timestampForSlot(slot, coachTimezone);
    if (!timestamp) return slot;
...
export function getActiveSlots(coach: CoachProfile) {
...
export async function readScheduledAppointments(): Promise<BookedSession[]> {
    ...
    try {
        ...
        const sessions = JSON.parse(contents) as BookedSession[];
...
export async function readData(): Promise<SchedulerData> {
    ...
    try {
        ...
        const data = JSON.parse(contents) as SchedulerData;
        data.coaches ??= [];
        data.players ??= [];
        data.sessions ??= [];
        ...
        for (const coach of data.coaches) {
            ...
            coach.slots = coach.slots.map(slot =>
                isDatedSlot(slot) ? slot : buildDatedSlot(slot)
            );
        }
        ...
        if (err.code === 'ENOENT') {
            const initial: SchedulerData = { coaches: [], players: [], sessions: [] };
...
export function buildCoachEmbed(coach: CoachProfile) {
...
function getSlotDay(slot: string) {
    return slot.split(' ')[0];
}

export function buildScheduleDayButtons(coach: CoachProfile) {
    const activeSlots = getActiveSlots(coach);
    const availableDays = availabilityDays.filter(day =>
        activeSlots.some(slot => getSlotDay(slot) === day)
    );
    ...
    return Array.from({ length: Math.ceil(availableDays.length / 5) }, (_, index) => {
        ...
        row.addComponents(
            ...availableDays.slice(index * 5, index * 5 + 5).map(day => {
...
export function buildSlotSelectMenu(
    coach: CoachProfile,
...
export function buildScheduleButton(coach: CoachProfile) {
    const row = new ActionRowBuilder<ButtonBuilder>();
    row.addComponents(
        new ButtonBuilder()
...
export function buildTimeSelectionMenu(day: string) {
    const row = new ActionRowBuilder<StringSelectMenuBuilder>();
    const menu = new StringSelectMenuBuilder()
...
function formatAppointmentLabel(session: BookedSession) {
    return `${session.coachUsername} • ${session.slot}`;
}

export function buildAppointmentCancellationMenu(sessions: BookedSession[]) {
    const row = new ActionRowBuilder<StringSelectMenuBuilder>();
    const menu = new StringSelectMenuBuilder()
...
export function buildAppointmentCancellationConfirmationButtons(sessionId: string) {
    const row = new ActionRowBuilder<ButtonBuilder>();
    row.addComponents(
        new ButtonBuilder()
...
export function buildMondayRestoreButtons(coachId: string) {
    const row = new ActionRowBuilder<ButtonBuilder>();
    row.addComponents(
        new ButtonBuilder()
...
export async function notifyRequesterOfDecision(
    ...
    requestInteraction: Interaction | undefined,
    guildId: string | undefined
) {
    ...
    try {
        ...
        console.error(
            `Failed to DM requester ${requesterId}. Falling back to ephemeral reply.`,
            err
        );
    }

    if (!requestInteraction?.isRepliable()) return;
    ...
    try {
        ...
        console.error(
            `Failed to fallback notify requester ${requesterId} via interaction reply`,
            fallbackErr
        );
    }
}
...
export async function notifyAvailabilityEmpty(coach: CoachProfile) {
    try {
        await sendDmToUser(coach.userId, coach.guildId, {
            embeds: [
                new EmbedBuilder()
                    ...
                    .setDescription(
                        'All of your available coaching slots have been booked. Would you like your saved availability restored next Monday?'
                    )
                    .setColor(0x57f287)
            ],
    ...
    new SlashCommandBuilder()
        ...
        .addStringOption((option) =>
            option
                .setName('visibility')
                .setDescription(
                    'Show your coaching profile on this server only or on all servers'
                )
                .setRequired(true)
                .addChoices(
...
client.on('interactionCreate', async (interaction) => {
    ...
    if (interaction.isChatInputCommand()) {
        ...
        try {
            ...
            if (commandName === 'update-availability') {
                const data = await readData();
                const profile = data.coaches.find(coach =>
                    coach.userId === interaction.user.id &&
            ...
            if (commandName === 'coach-list') {
                ...
                const visibleCoaches = data.coaches.filter(coach =>
                    coach.visibility === 'all' ||
            ...
            if (commandName === 'coach-remove') {
                ...
                data.coaches = data.coaches.filter(coach => {
                    if (coach.userId !== interaction.user.id) return true;
            ...
            if (commandName === 'cancel-appointment') {
                ...
                const sessions = data.sessions.filter(session =>
                    session.playerId === interaction.user.id || coachIds.includes(session.coachId)
                );
    ...
    if (interaction.isButton()) {
        ...
        if (interaction.customId === 'ui-player-browse') {
            ...
            const visibleCoaches = data.coaches.filter(coach =>
                coach.visibility === 'all' || coach.guildId === interaction.guildId
            );
        ...
        if (type === 'schedule-confirm') {
            ...
            const notified = await notifyCoachOfRequest(
                ...
                requestId,
                pendingSelection.channelId,
                pendingSelection.guildId
            );
        ...
        if (type === 'request-action') {
            ...
            if (action === 'accept') {
                ...
                await notifyRequesterOfDecision(
                    pendingRequest.requesterId,
                    pendingRequest.coachUsername,
                    pendingRequest.slot,
                    true,
                    undefined,
                    pendingRequest.requestInteraction,
                    pendingRequest.guildId
                );
 ...
 */
export function coachHasNoActiveSlots(coach: CoachProfile): boolean {
 ...
 */
export function restoreCoachAvailability(coach: CoachProfile): void {
 ...
 */
export async function restoreAvailabilityAndSave(
    coachId: string
...
export async function processScheduledMondayRestores(now = new Date()) {
    ...
    for (const coach of data.coaches) {
        ...
        restoreSavedAvailability(
            coach,
            slot => isSlotExpiredToday(slot, now),
            template => buildDatedSlot(template, now)
        );
        coach.scheduledRestoreDate = undefined;
        changed = true;
    }
